...
class Backup_db extends AdminController {  
	...
	function delete($id=''){
		...
		redirect($this->own_link."?msg=".urlencode('Delete backup data success')."&type_msg=success");
	}
	...
	function restore($file_name=""){
		...
		if(trim($id) != ''){
			...
			redirect($this->own_link."?msg=".urlencode('Restore Database success')."&type_msg=success");
		}else{
			redirect($this->own_link."?msg=".urlencode('Invalid file name.')."&type_msg=error");
		}
	}
	...
	function backup(){
		...
		$this->db->query($query);
		redirect($this->own_link."?msg=".urlencode('Backup Database success')."&type_msg=success");
	}
